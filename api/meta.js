...
async function handleTikTokLoginDebug(req, res) {
  ...
  return json(res, 200, {
    ok: true,
    deployed: 'meta-whatsapp-mersal-image-send-v67',
...
async function publishWhatsAppMersalFromReady(body, message) {
  ...
  const phones = whatsappRecipientsFromBody(body);
  if (!phones.length) throw new Error('WhatsApp recipient phone is missing.');
  const mediaUrl = String(body.mediaUrl || body.finalFileUrl || '').trim();
...
async function publishReadyPayload(body, req = null) {
  ...
  return { ok, deployed: 'meta-whatsapp-mersal-image-send-v67', taskId: body.taskId || null, results, publishedAt: new Date().toISOString() };
}
...
async function handleReadyPublish(req, res) {
  ...
  try {
    ...
    return json(res, 400, { ok: false, deployed: 'meta-whatsapp-mersal-image-send-v67', error: error.message || 'Publish failed' });
  }
}
...
async function handleAutoPublish(req, res) {
  ...
  return json(res, 200, { ok: true, deployed: 'meta-whatsapp-mersal-image-send-v67', checked: all.length, due: due.length, processed, ranAt: now.toISOString() });
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    return json(res, 404, { ok: false, error: 'Unknown API route', path, deployed: 'meta-whatsapp-mersal-image-send-v67' });
  } catch (error) {
