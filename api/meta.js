...
const TIKTOK_DEFAULT_SCOPES = ['user.info.basic', 'video.upload', 'video.publish'];
...
const PLATFORM_CONNECTIONS_COLLECTION = process.env.PLATFORM_CONNECTIONS_COLLECTION || 'platform_connections';
...
function sendTikTokCallbackInfoPage(res) {
  res.statusCode = 200;
...
<head>
  ...
  <title>MZJ Publish</title>
  <link rel="icon" type="image/png" href="/assets/favicon.png" />
  <style>
    body{margin:0;font-family:Arial,Helvetica,sans-serif;background:#f8fafc;color:#0f172a;line-height:1.6}
    main{max-width:760px;margin:0 auto;padding:48px 20px}
    .card{background:#fff;border:1px solid #e2e8f0;border-radius:20px;padding:28px;box-shadow:0 10px 28px rgba(15,23,42,.06)}
    img{width:84px;height:84px;object-fit:contain;border:1px solid #e2e8f0;border-radius:18px;padding:8px;background:#fff}
    h1{margin:14px 0 8px;font-size:32px}.muted{color:#475569}a{color:#4a2417;font-weight:700;text-decoration:none}
  </style>
</head>
<body>
  <main>
    <section class="card">
      <img src="/assets/mzj-publish-app-icon-1024.png" alt="MZJ Publish app icon" />
      <h1>MZJ Publish</h1>
      <p class="muted">TikTok OAuth callback endpoint for MZJ Publish.</p>
      <p>This endpoint is used after a user authorizes TikTok Login. To connect TikTok, start from the MZJ Publish dashboard and click Connect TikTok.</p>
      <p><a href="/">Open MZJ Publish</a> · <a href="/privacy-policy.html">Privacy Policy</a> · <a href="/terms-of-service.html">Terms of Service</a></p>
    </section>
  </main>
</body>
</html>`);
...
async function handleTikTokLoginDebug(req, res) {
  ...
  return json(res, 200, {
    ok: true,
    deployed: 'meta-whatsapp-template-caption-v71',
...
async function handleYouTubeCallback(req, res) {
  ...
  await savePlatformConnection('youtube', {
    source:'youtube-oauth-callback', connected:true, status:'connected', accessToken:tokenPayload.access_token || '', refreshToken:tokenPayload.refresh_token || '', tokenPayload, hasToken:true, tokenStored:true, scopes:String(tokenPayload.scope || '').split(/\s+/).filter(Boolean), accountId:channel?.id || '', channelId:channel?.id || '', accountName:channel?.title || '', channelTitle:channel?.title || '', connectedAtIso:connectedAt
  }).catch(error => console.warn('YouTube central connection save error', error.message || error));
...
async function handleYouTubeLogout(req, res) {
  await savePlatformConnection('youtube', { connected:false, status:'disconnected', state:'warning', hasToken:false, tokenStored:false, disconnectedAtIso:new Date().toISOString() }).catch(()=>null);
...
async function handleCallback(req, res) {
  ...
  await saveMetaConnectionFromToken(longToken.access_token, 'meta-oauth-callback').catch(error => console.warn('Meta central connection save error', error.message || error));
...
async function handlePages(req, res) {
  ...
  try {
    ...
    saveMetaConnectionFromToken(token, 'meta-pages-status').catch(error => console.warn('Meta central connection save error', error.message || error));
...
async function firestoreList(collection) {
  const response = await fetch(firestoreUrl(`${collection}?pageSize=1000`));
  const payload = await response.json().catch(() => ({}));
...
async function firestoreGet(collection, docId) {
  const response = await fetch(firestoreUrl(`${collection}/${encodeURIComponent(docId)}`));
  const payload = await response.json().catch(() => ({}));
  if (!response.ok) {
    if (response.status === 404) return null;
    throw new Error(payload.error?.message || `Firestore get error: ${response.status}`);
  }
  return { id: firestoreDocId(payload.name), name: payload.name, ...fromFirestoreFields(payload.fields || {}) };
}
...
function publicConnectionPayload(data = {}) {
  const clean = { ...data };
  ['accessToken','refreshToken','pageAccessToken','userAccessToken','clientSecret','appSecret','tokenPayload'].forEach(key => {
    if (clean[key]) clean[key] = '[REDACTED]';
  });
  if (Array.isArray(clean.pages)) clean.pages = clean.pages.map(page => ({ ...page, access_token: page.access_token ? '[REDACTED]' : page.access_token, pageAccessToken: page.pageAccessToken ? '[REDACTED]' : page.pageAccessToken }));
  return clean;
}

async function savePlatformConnection(platform, data = {}) {
  const id = normalizePlatformApi(platform || data.platform || 'platform') || String(platform || data.platform || 'platform').toLowerCase();
  const now = new Date().toISOString();
  const payload = {
    platform: id,
    connected: data.connected !== false,
    status: data.status || (data.connected === false ? 'disconnected' : 'connected'),
    state: data.state || (data.connected === false ? 'error' : 'ready'),
    source: data.source || 'oauth-callback',
    updatedAtIso: now,
    connectedAtIso: data.connectedAtIso || data.connected_at || now,
    ...data
  };
  try {
    await firestorePatch(PLATFORM_CONNECTIONS_COLLECTION, id, payload);
  } catch (error) {
    try { await firestoreCreate(PLATFORM_CONNECTIONS_COLLECTION, payload, id); }
    catch (createError) { console.warn('Platform connection save failed', id, createError.message || createError); }
  }
  return payload;
}

async function saveMetaConnectionFromToken(userToken, source = 'meta-oauth') {
  if (!userToken) return null;
  const pages = await safe(() => getPages(userToken));
  const pageList = pages.ok && Array.isArray(pages.result) ? pages.result : [];
  const firstPage = pageList[0] || null;
  const firstInstagram = pageList.map(page => page.instagram_business_account).find(Boolean) || null;
  const facebookPayload = await savePlatformConnection('facebook', {
    source,
    connected: true,
    status: 'connected',
    ...
    pagesCount: pageList.length,
    pagesError: pages.ok ? '' : pages.error
  });
  if (firstInstagram) {
    await savePlatformConnection('instagram', {
      source,
      connected: true,
      status: 'connected',
      hasToken: true,
      tokenStored: true,
      accountId: firstInstagram.id || '',
      accountName: firstInstagram.username || firstInstagram.name || firstInstagram.id || '',
      username: firstInstagram.username || '',
      igUserId: firstInstagram.id || '',
      pageId: firstPage?.id || '',
      pageName: firstPage?.name || '',
      userAccessToken: userToken,
      accessToken: userToken,
    ...
    await savePlatformConnection('instagram', {
      source,
      connected: false,
      status: pageList.length ? 'missing_instagram_business' : 'disconnected',
      state: 'warning',
      hasToken: false,
      tokenStored: false,
      pagesCount: pageList.length,
...
async function handlePlatformConnectionsStatus(req, res) {
  const docs = await firestoreList(PLATFORM_CONNECTIONS_COLLECTION).catch(() => []);
  return json(res, 200, { ok: true, connections: docs.map(publicConnectionPayload) });
}
...
function platformPublishMetaApi(body, platform) {
  const normalized = normalizePlatformApi(platform);
  const list = Array.isArray(body.platformPublishing) ? body.platformPublishing : (Array.isArray(body.taskSnapshot?.platformPublishing) ? body.taskSnapshot.platformPublishing : []);
  const match = list.find(item => normalizePlatformApi(item.platform) === normalized) || null;
  const platformTypes = body.platformTypes || body.taskSnapshot?.platformTypes || {};
  const rawType = match?.postType || platformTypes[platform] || platformTypes[normalized] || body.postType || body.publishType || '';
  const postType = normalizePostTypeApi(rawType);
  return {
    postType,
    postTypeLabel: match?.postTypeLabel || body.postTypeLabel || postTypeLabelApi(postType),
    requiredDimensions: match?.requiredDimensions || body.requiredDimensions || null
  };
}
function bodyForPlatformApi(body, platform) {
  const meta = platformPublishMetaApi(body, platform);
  return { ...body, postType: meta.postType, postTypeLabel: meta.postTypeLabel, requiredDimensions: meta.requiredDimensions };
}
...
function taskPayloadForPublishing(task) {
  ...
  return {
    ...
    platformTypes: task.platformTypes || snapshot.platformTypes || {},
    platformPublishing: Array.isArray(task.platformPublishing) ? task.platformPublishing : (Array.isArray(snapshot.platformPublishing) ? snapshot.platformPublishing : []),
    platforms,
    ...
    whatsappPhones: task.whatsappPhones || snapshot.whatsappPhones || []
  };
}
...
async function savePublishResultsToLogs(task, publishResponse, mode = 'auto') {
  ...
  for (const platformResult of results) {
    ...
    writes.push(firestoreCreate('publish_logs', {
      ...
      platform,
      platforms: [platform],
      type: platformPublishMetaApi(task, platform).postType || task.contentType || task.taskSnapshot?.contentType || task.type || '',
      postType: platformPublishMetaApi(task, platform).postType,
      postTypeLabel: platformPublishMetaApi(task, platform).postTypeLabel,
      requiredDimensions: platformPublishMetaApi(task, platform).requiredDimensions,
      mode,
...
function mediaUrlsFromReadyBody(body) {
  const raw = Array.isArray(body.mediaUrls) ? body.mediaUrls : (Array.isArray(body.fileUrls) ? body.fileUrls : []);
  const urls = raw.map(item => typeof item === 'string' ? item : (item?.url || item?.mediaUrl || item?.fileUrl || item?.downloadURL || '')).map(item => String(item || '').trim()).filter(Boolean);
  const first = String(body.mediaUrl || body.finalFileUrl || '').trim();
  if (first && !urls.includes(first)) urls.unshift(first);
  return [...new Set(urls)];
}
function mediaUrlLooksVideo(url, body = {}) {
  return isProbablyVideo(url, body.mimeType || body.fileType || '', body.contentType || body.type || '');
}

async function postFacebookStoryVideoUpload(uploadUrl, pageToken, mediaUrl) {
  ...
  const payload = await response.json().catch(() => ({}));
  if (!response.ok || payload.error) {
    throw new Error(payload.error?.message || `Facebook video story upload failed: ${response.status} ${JSON.stringify(payload)}`);
  }
  return payload;
}

async function publishFacebookStoryFromReady(ctx, body, message) {
  const mediaUrls = mediaUrlsFromReadyBody(body);
  const mediaUrl = mediaUrls[0] || '';
  if (!mediaUrl) throw new Error('Facebook Story requires a media URL.');
  if (mediaUrls.length > 1) throw new Error('Facebook Story supports one media file per story job. Split story files into separate jobs.');
  ...
  if (isVideo) {
    const start = await graphPost(`/${ctx.page.id}/video_stories`, { upload_phase: 'start' }, ctx.pageToken);
    const videoId = start.video_id || start.id;
    const uploadUrl = start.upload_url || start.uploadUrl;
    if (!videoId || !uploadUrl) throw new Error(`Facebook video story start did not return video_id/upload_url. response=${JSON.stringify(start)}`);
    const upload = await postFacebookStoryVideoUpload(uploadUrl, ctx.pageToken, mediaUrl);
    const finish = await graphPost(`/${ctx.page.id}/video_stories`, { upload_phase: 'finish', video_id: videoId }, ctx.pageToken);
    return { ok: true, platform: 'facebook', type: 'story_video', postType: 'story', id: finish.post_id || finish.id || videoId, start, upload, publish: finish };
  }

  const photo = await graphPost(`/${ctx.page.id}/photos`, { url: mediaUrl, published: false }, ctx.pageToken);
  const photoId = photo.id || photo.photo_id;
  if (!photoId) throw new Error(`Facebook Story photo upload did not return a photo id. response=${JSON.stringify(photo)}`);
  const publish = await graphPost(`/${ctx.page.id}/photo_stories`, { photo_id: photoId }, ctx.pageToken);
  return { ok: true, platform: 'facebook', type: 'story_photo', postType: 'story', id: publish.post_id || publish.id || photoId, upload: photo, publish };
}
...
async function publishFacebookFromReady(ctx, body, message) {
  ...
  if (postType === 'story') {
    return publishFacebookStoryFromReady(ctx, body, message);
  }
  if (!mediaUrl) {
  ...
  if (mediaUrls.length > 1 && mediaUrls.every(url => !mediaUrlLooksVideo(url, body))) {
    const attached_media = [];
    for (const url of mediaUrls) {
      const upload = await graphPost(`/${ctx.page.id}/photos`, { url, published: false }, ctx.pageToken);
      const media_fbid = upload.id || upload.post_id || upload.photo_id;
      if (media_fbid) attached_media.push({ media_fbid });
    }
    if (!attached_media.length) throw new Error('Facebook did not create any unpublished photo for carousel post.');
    const result = await graphPost(`/${ctx.page.id}/feed`, { message, attached_media }, ctx.pageToken);
    return { ok: true, platform: 'facebook', type: 'carousel_photos', postType: postType || 'photo_post', mediaCount: attached_media.length, result };
  }
...
async function publishInstagramFromReady(ctx, body, message) {
  ...
  const isVideo = isProbablyVideo(mediaUrl, mimeType, contentType);
  const postType = normalizePostTypeApi(body.postType || body.publishType || '');
  if (mediaUrls.length > 1 && postType !== 'story' && mediaUrls.every(url => !mediaUrlLooksVideo(url, body))) {
    const children = [];
    for (const url of mediaUrls) {
      const child = await graphPost(`/${igId}/media`, { image_url: url, is_carousel_item: true }, ctx.pageToken);
      const childId = child.id || child.creation_id;
      if (childId) children.push(childId);
    }
    if (!children.length) throw new Error('Instagram did not create any carousel child media.');
    const create = await graphPost(`/${igId}/media`, { media_type: 'CAROUSEL', caption: message, children: children.join(',') }, ctx.pageToken);
    const creationId = create.id || create.creation_id;
    if (!creationId) throw new Error('Instagram did not return a carousel creation id.');
    const publish = await graphPost(`/${igId}/media_publish`, { creation_id: creationId }, ctx.pageToken);
    return { ok: true, platform: 'instagram', type: 'carousel', postType: postType || 'photo_post', mediaCount: children.length, create, publish };
  }
  ...
  const creationId = create.id || create.creation_id;
  if (!creationId) {
    throw new Error(`Instagram Media ID is not available. createParams=${JSON.stringify(createParams)} response=${JSON.stringify(create)}`);
  }
  const publish = await graphPost(`/${igId}/media_publish`, { creation_id: creationId }, ctx.pageToken);
...
function normalizeMersalEndpoint(value = '') {
  return String(value || 'https://w-mersal.com').trim().replace(/\/$/, '') || 'https://w-mersal.com';
}

async function getMersalSettings(body = {}) {
  const settings = await firestoreGet('system_settings', 'main').catch(() => null);
  const stored = settings?.mersal || settings?.whatsappMersal || {};
  const inline = body.mersal || body.whatsappMersal || {};
  return {
    ...
    imageTemplate: String(inline.imageTemplate || stored.imageTemplate || settings?.mersalImageTemplate || 'mzj_image_caption_v4').trim(),
...
function normalizeMessagePartApi(value = '') {
  return String(value || '').replace(/\r\n/g, '\n').trim();
}

function buildWhatsappMessageApi(body = {}, fallbackMessage = '') {
  const caption = normalizeMessagePartApi(body.caption || body.postCaption || body.messageCaption || body.copy || '');
  const rawTags = body.hashtagsText || body.hashtagText || body.hashtags || body.hashTags || body.tags || '';
  const tags = Array.isArray(rawTags) ? rawTags.join(' ') : normalizeMessagePartApi(rawTags);
  const merged = [caption, tags].filter(Boolean).join('\n\n').trim();
  return merged || normalizeMessagePartApi(fallbackMessage || body.message || body.whatsappMessage || body.title || '');
}
...
function whatsappRecipientsFromBody(body = {}) {
  const values = [];
  const push = value => {
    if (Array.isArray(value)) value.forEach(push);
    else if (value && typeof value === 'object') push(value.phone || value.mobile || value.number || value.whatsapp || value.value);
    else if (value !== undefined && value !== null) String(value).split(/[,،\n]+/).forEach(part => values.push(part));
  };
  push(body.phones || body.recipients || body.whatsappPhones || body.phone || body.customerPhone || body.clientPhone || body.taskSnapshot?.phones || body.taskSnapshot?.phone || '');
  return [...new Set(values.map(normalizeWhatsappPhone).filter(Boolean))];
}

async function postMersalForm(url, fields = {}, fileField = null) {
  const form = new FormData();
  Object.entries(fields || {}).forEach(([key, value]) => form.append(key, String(value ?? '')));
  if (fileField) form.append(fileField.name, fileField.blob, fileField.filename);
  const response = await fetch(url, { method: 'POST', body: form });
  const payload = await response.json().catch(() => ({}));
  if (!response.ok || (payload.status && String(payload.status).toLowerCase() !== 'success')) {
    throw new Error(payload.message || payload.error || `Mersal API error: ${response.status}`);
  }
  return payload;
}

async function postMersalJson(url, payload = {}) {
  ...
  if (!response.ok || (result.status && String(result.status).toLowerCase() !== 'success')) {
    const errorText = result.message || result.error || (result.errors ? JSON.stringify(result.errors) : '') || `Mersal API error: ${response.status}`;
    throw new Error(errorText);
  }
  return result;
}

async function mediaBlobFromUrl(url, fallbackName = 'mzj-media') {
  const response = await fetch(url);
  if (!response.ok) throw new Error(`Could not download WhatsApp media: ${response.status}`);
  const buffer = await response.arrayBuffer();
  const contentType = response.headers.get('content-type') || 'application/octet-stream';
  const cleanName = String(fallbackName || 'mzj-media').replace(/[\\/:*?"<>|]/g, '-');
  return { blob: new Blob([buffer], { type: contentType }), contentType, filename: cleanName || 'mzj-media' };
}


async function loadImportedWhatsappPhones() {
  try {
    const contacts = await firestoreList('whatsapp_contacts');
    return contacts
      .filter(item => item.active !== false)
      .map(item => normalizeWhatsappPhone(item.phone || item.number || item.mobile || item.id || ''))
      .filter(Boolean);
  } catch (error) {
    console.warn('Unable to load imported WhatsApp contacts', error.message || error);
    return [];
  }
}

async function handleMersalValidate(req, res) {
  if (req.method !== 'POST') return json(res, 405, { ok: false, error: 'Method not allowed' });
  const body = await readBody(req);
  const apiEndpoint = normalizeMersalEndpoint(body.apiEndpoint || 'https://w-mersal.com');
  const token = String(body.token || '').trim();
  if (!token) return json(res, 400, { ok: false, connected: false, error: 'Mersal token is missing.' });
  const url = `${apiEndpoint}/api/wpbox/getTemplates?token=${encodeURIComponent(token)}`;
  const response = await fetch(url);
  const payload = await response.json().catch(() => ({}));
  if (!response.ok || (payload.status && String(payload.status).toLowerCase() !== 'success')) {
    return json(res, 400, { ok: false, connected: false, error: payload.message || payload.error || `Mersal validation failed: ${response.status}` });
  }
  return json(res, 200, { ok: true, connected: true, templates: Array.isArray(payload.templates) ? payload.templates.length : 0 });
}

async function publishWhatsAppMersalFromReady(body, message) {
  message = buildWhatsappMessageApi(body, message || buildPublishMessageApi(body) || '');
  const settings = await getMersalSettings(body);
  if (!settings.token) throw new Error('Mersal token is missing. Save Mersal settings first.');
  let phones = whatsappRecipientsFromBody(body);
  if (!phones.length) phones = await loadImportedWhatsappPhones();
  if (!phones.length) throw new Error('WhatsApp recipient phone is missing. Import customer numbers or add a recipient phone.');
  const mediaUrl = String(body.mediaUrl || body.finalFileUrl || '').trim();
  const mimeType = String(body.mimeType || body.fileType || '').toLowerCase();
  const contentType = String(body.contentType || body.type || '').toLowerCase();
  const postType = normalizePostTypeApi(body.postType || body.publishType || '');
  const isVideo = isProbablyVideo(mediaUrl, mimeType, contentType) || postType === 'whatsapp_video_pending';
  const sent = [];
  const templateEndpoint = `${settings.apiEndpoint}/api/wpbox/sendtemplatemessage`;
  const textEndpoint = `${settings.apiEndpoint}/api/wpbox/sendmessage`;

  if (isVideo) {
    return { ok: false, platform: 'whatsapp', type: 'video', postType, skipped: true, error: 'Mersal video API is still not confirmed. Image sending is enabled only.' };
  }

  if (mediaUrl) {
    const templateName = settings.imageTemplate || 'mzj_image_caption_v4';
    const templateLanguage = settings.templateLanguage || 'ar';
    const captionText = normalizeMessagePartApi(body.caption || body.postCaption || body.messageCaption || body.copy || body.title || '');
    const rawTags = body.hashtagsText || body.hashtagText || body.hashtags || body.hashTags || body.tags || '';
    const hashtagText = Array.isArray(rawTags) ? normalizeMessagePartApi(rawTags.join(' ')) : normalizeMessagePartApi(rawTags);
    const bodyParameters = [
      { type: 'text', text: captionText || message || body.title || 'MZJ' },
      { type: 'text', text: hashtagText || ' ' }
    ];
    for (const phone of phones) {
      ...
      const result = await postMersalJson(templateEndpoint, payload);
      sent.push({ phone, message_id: result.message_id || null, message_wamid: result.message_wamid || null, template_name: templateName, raw: result });
    }
    return { ok: true, platform: 'whatsapp', type: 'image_template_v4', postType: postType || 'whatsapp_image', id: sent[0]?.message_wamid || sent[0]?.message_id || '', sent };
  }

  for (const phone of phones) {
    const result = await postMersalForm(textEndpoint, { token: settings.token, phone, message: message || body.title || 'MZJ', caption: message || body.title || 'MZJ', text: message || body.title || 'MZJ' });
    sent.push({ phone, message_id: result.message_id || null, message_wamid: result.message_wamid || null, raw: result });
  }
  return { ok: true, platform: 'whatsapp', type: 'text', postType: postType || 'whatsapp_text', id: sent[0]?.message_wamid || sent[0]?.message_id || '', sent };
}
...
async function publishReadyPayload(body, req = null) {
  ...
  let ctx = null;
  const getCtx = async () => { if (!ctx) ctx = await getConfiguredPublishingContext(); return ctx; };
  const results = [];
  for (const platform of platforms) {
    try {
      if (['facebook', 'fb', 'فيسبوك'].includes(platform)) {
        results.push(await publishFacebookFromReady(await getCtx(), bodyForPlatformApi(body, platform), message));
      } else if (['instagram', 'ig', 'انستجرام', 'انستغرام'].includes(platform)) {
        results.push(await publishInstagramFromReady(await getCtx(), bodyForPlatformApi(body, platform), message));
      } else if (['whatsapp', 'واتساب', 'حملات واتساب'].includes(platform)) {
        results.push(await publishWhatsAppMersalFromReady(bodyForPlatformApi(body, platform), message));
      } else if (['youtube', 'yt', 'يوتيوب'].includes(platform)) {
        if (!req) throw new Error('YouTube publish requires browser connection context.');
        results.push(await publishYouTubeFromReady(req, bodyForPlatformApi(body, platform), message));
      } else if (['tiktok', 'تيك توك'].includes(platform)) {
        results.push({ ok: false, platform: 'tiktok', type: platformPublishMetaApi(body, platform).postType, postType: platformPublishMetaApi(body, platform).postType, skipped: true, error: 'TikTok auto publish is not enabled yet. Current connection is Sandbox/Draft mode.' });
      } else if (['snapchat', 'snap', 'سناب'].includes(platform)) {
        results.push({ ok: false, platform: 'snapchat', type: platformPublishMetaApi(body, platform).postType, postType: platformPublishMetaApi(body, platform).postType, skipped: true, error: 'Snapchat official publishing is pending Public Profile API allowlist approval.' });
      } else {
  ...
  return { ok, deployed: 'meta-whatsapp-template-caption-v71', taskId: body.taskId || null, results, publishedAt: new Date().toISOString() };
}
...
async function handleReadyPublish(req, res) {
  ...
  try {
    ...
    return json(res, 400, { ok: false, deployed: 'meta-whatsapp-template-caption-v71', error: error.message || 'Publish failed' });
  }
}
...
async function handleTikTokCallback(req, res) {
  const { code, state, error, error_description } = req.query || {};
  if (error) throw new Error(error_description || error);
  if (!code) return sendTikTokCallbackInfoPage(res);
  const cookies = parseCookies(req);
  ...
  let tiktokUser = null;
  try { tiktokUser = await getTikTokUserInfo(cleanPayload.access_token); } catch (_) {}
  await savePlatformConnection('tiktok', {
    source:'tiktok-oauth-callback', connected:true, status:'connected', accessToken:cleanPayload.access_token, refreshToken:cleanPayload.refresh_token || '', tokenPayload:cleanPayload, hasToken:true, tokenStored:true, scopes:String(cleanPayload.scope || '').split(',').filter(Boolean), accountId:cleanPayload.open_id || tiktokUser?.open_id || '', accountName:tiktokUser?.display_name || tiktokUser?.username || cleanPayload.open_id || '', username:tiktokUser?.username || '', user:tiktokUser || null, connectedAtIso:cleanPayload.connected_at
  }).catch(error => console.warn('TikTok central connection save error', error.message || error));
...
async function handleTikTokLogout(req, res) {
  await savePlatformConnection('tiktok', { connected:false, status:'disconnected', state:'warning', hasToken:false, tokenStored:false, disconnectedAtIso:new Date().toISOString() }).catch(()=>null);
...
async function handleAutoPublish(req, res) {
  ...
  return json(res, 200, { ok: true, deployed: 'meta-whatsapp-template-caption-v71', checked: all.length, due: due.length, processed, ranAt: now.toISOString() });
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (path === 'logout') { await savePlatformConnection('facebook', { connected:false, status:'disconnected', state:'error', hasToken:false, tokenStored:false, disconnectedAtIso:new Date().toISOString() }).catch(()=>null); await savePlatformConnection('instagram', { connected:false, status:'disconnected', state:'error', hasToken:false, tokenStored:false, disconnectedAtIso:new Date().toISOString() }).catch(()=>null); res.setHeader('Set-Cookie', clearCookie(TOKEN_COOKIE)); return json(res, 200, { ok: true, connected: false }); }
    if (path === 'pages') return await handlePages(req, res);
    if (path === 'platform-connections/status') return await handlePlatformConnectionsStatus(req, res);
    ...
    if (path === 'mersal/validate') return await handleMersalValidate(req, res);
    ...
    return json(res, 404, { ok: false, error: 'Unknown API route', path, deployed: 'meta-whatsapp-template-caption-v71' });
  } catch (error) {
