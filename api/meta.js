...
async function handleTikTokLoginDebug(req, res) {
  ...
  return json(res, 200, {
    ok: true,
    deployed: 'meta-whatsapp-caption-icons-v69',
...
async function publishWhatsAppMersalFromReady(body, message) {
  message = String(message || buildPublishMessageApi(body) || '').trim();
  ...
  if (mediaUrl) {
    ...
    for (const phone of phones) {
      const result = await postMersalForm(endpoint, { token: settings.token, phone, message: message || body.title || 'MZJ' }, { name: 'image', blob: file.blob, filename: file.filename });
  ...
  for (const phone of phones) {
    const result = await postMersalForm(endpoint, { token: settings.token, phone, message: message || body.title || 'MZJ' });
    sent.push({ phone, message_id: result.message_id || null, message_wamid: result.message_wamid || null, raw: result });
  }
...
async function publishReadyPayload(body, req = null) {
  ...
  return { ok, deployed: 'meta-whatsapp-caption-icons-v69', taskId: body.taskId || null, results, publishedAt: new Date().toISOString() };
}
...
async function handleReadyPublish(req, res) {
  ...
  try {
    ...
    return json(res, 400, { ok: false, deployed: 'meta-whatsapp-caption-icons-v69', error: error.message || 'Publish failed' });
  }
}
...
async function handleAutoPublish(req, res) {
  ...
  return json(res, 200, { ok: true, deployed: 'meta-whatsapp-caption-icons-v69', checked: all.length, due: due.length, processed, ranAt: now.toISOString() });
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    return json(res, 404, { ok: false, error: 'Unknown API route', path, deployed: 'meta-whatsapp-caption-icons-v69' });
  } catch (error) {
