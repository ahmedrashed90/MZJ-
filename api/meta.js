...
function redactSecrets(value) {
  ...
  if (value && typeof value === 'object') {
    ...
    for (const [key, item] of Object.entries(value)) {
      if (/access[_-]?token/i.test(key) || /token/i.test(key)) {
...
async function handleDebug(req, res) {
  ...
  return json(res, 200, {
    ok: true,
    deployed: 'meta-secure-debug-v13',
...
async function handlePagesDebug(req, res) {
  ...
  return json(res, 200, {
    ...
    connected: true,
    deployed: 'meta-secure-debug-v13',
    note: 'Debug output is redacted. Tokens are never returned by this endpoint.',
    me: redactSecrets(me),
    permissions: redactSecrets(permissions),
    directAccounts: redactSecrets(directAccounts),
    nestedAccounts: redactSecrets(nestedAccounts),
    businesses: redactSecrets(businesses)
  });
}
async function handlePageDirectDebug(req, res) {
  ...
  return json(res, 200, {
    ...
    me: redactSecrets(me),
    permissions: redactSecrets(permissions),
    page: redactSecrets(page),
    configuredPage: redactSecrets(configuredPage),
    ...
    configuredFeedRead: redactSecrets(configuredFeedRead)
  });
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    return json(res, 404, { ok: false, error: 'Unknown API route', path, deployed: 'meta-secure-debug-v13' });
  } catch (error) {
