...
async function handleTikTokLoginDebug(req, res) {
  ...
  return json(res, 200, {
    ok: true,
    deployed: 'meta-whatsapp-caption-icons-v70',
...
async function getMersalSettings(body = {}) {
  ...
  return {
    ...
    imageTemplate: String(inline.imageTemplate || stored.imageTemplate || settings?.mersalImageTemplate || 'mzj_image_campaign').trim(),
...
async function publishWhatsAppMersalFromReady(body, message) {
  ...
  const sent = [];
  const endpoint = `${settings.apiEndpoint}/api/wpbox/sendmessage`;
  if (isVideo) {
  ...
  if (mediaUrl) {
    const file = await mediaBlobFromUrl(mediaUrl, body.fileName || 'mzj-whatsapp-image.jpg');
    for (const phone of phones) {
      const result = await postMersalForm(endpoint, { token: settings.token, phone, message: message || body.title || 'MZJ', caption: message || body.title || 'MZJ', text: message || body.title || 'MZJ' }, { name: 'image', blob: file.blob, filename: file.filename });
      sent.push({ phone, message_id: result.message_id || null, message_wamid: result.message_wamid || null, raw: result });
    }
    return { ok: true, platform: 'whatsapp', type: 'image', postType: postType || 'whatsapp_image', id: sent[0]?.message_wamid || sent[0]?.message_id || '', sent };
  }
  for (const phone of phones) {
    const result = await postMersalForm(endpoint, { token: settings.token, phone, message: message || body.title || 'MZJ', caption: message || body.title || 'MZJ', text: message || body.title || 'MZJ' });
    sent.push({ phone, message_id: result.message_id || null, message_wamid: result.message_wamid || null, raw: result });
  }
...
async function publishReadyPayload(body, req = null) {
  ...
  return { ok, deployed: 'meta-whatsapp-caption-icons-v70', taskId: body.taskId || null, results, publishedAt: new Date().toISOString() };
}
...
async function handleReadyPublish(req, res) {
  ...
  try {
    ...
    return json(res, 400, { ok: false, deployed: 'meta-whatsapp-caption-icons-v70', error: error.message || 'Publish failed' });
  }
}
...
async function handleAutoPublish(req, res) {
  ...
  return json(res, 200, { ok: true, deployed: 'meta-whatsapp-caption-icons-v70', checked: all.length, due: due.length, processed, ranAt: now.toISOString() });
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    return json(res, 404, { ok: false, error: 'Unknown API route', path, deployed: 'meta-whatsapp-caption-icons-v70' });
  } catch (error) {
