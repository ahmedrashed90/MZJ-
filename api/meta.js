...
async function handleTikTokLoginDebug(req, res) {
  ...
  return json(res, 200, {
    ok: true,
    deployed: 'meta-v33-departments-lists-closed',
...
async function handleReadyPublish(req, res) {
  ...
  return json(res, ok ? 200 : 502, { ok, deployed: 'meta-v33-departments-lists-closed', taskId: body.taskId || null, results, publishedAt: new Date().toISOString() });
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    return json(res, 404, { ok: false, error: 'Unknown API route', path, deployed: 'meta-v33-departments-lists-closed' });
  } catch (error) {
