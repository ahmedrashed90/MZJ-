...
async function handleTikTokLoginDebug(req, res) {
  ...
  return json(res, 200, {
    ok: true,
    deployed: 'meta-structure-row-mapping-fix-v57',
...
async function publishReadyPayload(body, req = null) {
  ...
  return { ok, deployed: 'meta-structure-row-mapping-fix-v57', taskId: body.taskId || null, results, publishedAt: new Date().toISOString() };
}
...
async function handleReadyPublish(req, res) {
  ...
  try {
    ...
    return json(res, 400, { ok: false, deployed: 'meta-structure-row-mapping-fix-v57', error: error.message || 'Publish failed' });
  }
}
...
async function handleAutoPublish(req, res) {
  ...
  return json(res, 200, { ok: true, deployed: 'meta-structure-row-mapping-fix-v57', checked: all.length, due: due.length, processed, ranAt: now.toISOString() });
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    return json(res, 404, { ok: false, error: 'Unknown API route', path, deployed: 'meta-structure-row-mapping-fix-v57' });
  } catch (error) {
