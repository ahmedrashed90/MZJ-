...
async function handleTikTokLoginDebug(req, res) {
  ...
  return json(res, 200, {
    ok: true,
    deployed: 'meta-youtube-privacy-function-fix-v48',
...
async function savePublishResultsToLogs(task, publishResponse, mode = 'auto') {
  ...
  for (const platformResult of results) {
    ...
    writes.push(firestoreCreate('publish_logs', {
      ...
      platform,
      platforms: [platform],
      type: task.contentType || task.taskSnapshot?.contentType || task.type || '',
      mode,
...
async function publishYouTubeFromReady(req, body, message) {
  ...
  return { ok: true, platform: 'youtube', type: 'video', id: result.id, url: result.id ? `https://youtu.be/${result.id}` : '', result };
}
...
async function publishReadyPayload(body, req = null) {
  ...
  return { ok, deployed: 'meta-youtube-privacy-function-fix-v48', taskId: body.taskId || null, results, publishedAt: new Date().toISOString() };
}
...
async function handleReadyPublish(req, res) {
  ...
  try {
    ...
    return json(res, 400, { ok: false, deployed: 'meta-youtube-privacy-function-fix-v48', error: error.message || 'Publish failed' });
  }
}
...
async function handleAutoPublish(req, res) {
  ...
  return json(res, 200, { ok: true, deployed: 'meta-youtube-privacy-function-fix-v48', checked: all.length, due: due.length, processed, ranAt: now.toISOString() });
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    return json(res, 404, { ok: false, error: 'Unknown API route', path, deployed: 'meta-youtube-privacy-function-fix-v48' });
  } catch (error) {
