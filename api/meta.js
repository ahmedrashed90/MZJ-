...
async function handleTikTokLoginDebug(req, res) {
  ...
  return json(res, 200, {
    ok: true,
    deployed: 'meta-snapchat-pending-platform-v66',
...
async function publishReadyPayload(body, req = null) {
  ...
  for (const platform of platforms) {
    try {
      if (['facebook', 'fb', 'فيسبوك'].includes(platform)) {
        results.push(await publishFacebookFromReady(ctx, bodyForPlatformApi(body, platform), message));
      } else if (['instagram', 'ig', 'انستجرام', 'انستغرام'].includes(platform)) {
        results.push(await publishInstagramFromReady(ctx, bodyForPlatformApi(body, platform), message));
  ...
  return { ok, deployed: 'meta-snapchat-pending-platform-v66', taskId: body.taskId || null, results, publishedAt: new Date().toISOString() };
}
...
async function handleReadyPublish(req, res) {
  ...
  try {
    ...
    return json(res, 400, { ok: false, deployed: 'meta-snapchat-pending-platform-v66', error: error.message || 'Publish failed' });
  }
}
...
async function handleAutoPublish(req, res) {
  ...
  return json(res, 200, { ok: true, deployed: 'meta-snapchat-pending-platform-v66', checked: all.length, due: due.length, processed, ranAt: now.toISOString() });
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    return json(res, 404, { ok: false, error: 'Unknown API route', path, deployed: 'meta-snapchat-pending-platform-v66' });
  } catch (error) {
