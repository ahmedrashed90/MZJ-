...
async function handleDebug(req, res) {
  ...
  return json(res, 200, {
    ok: true,
    deployed: 'meta-page-token-derive-v12',
...
async function handlePagesDebug(req, res) {
  ...
  return json(res, 200, { ok: true, connected: true, deployed: 'meta-page-token-derive-v12', me, permissions, directAccounts, nestedAccounts, businesses });
}
...
async function handlePageDirectDebug(req, res) {
  ...
  const configuredPage = configuredToken ? await safe(() => graphGet(`/${pageId}`, { fields: 'id,name,category,link,fan_count,access_token,instagram_business_account{id,username,name}' }, configuredToken)) : { ok: false, error: 'No META_PAGE_ACCESS_TOKEN configured' };
  const derivedPageAccessToken = configuredPage.ok && configuredPage.result && configuredPage.result.access_token ? configuredPage.result.access_token : configuredToken;
  const configuredFeedRead = derivedPageAccessToken ? await safe(() => graphGet(`/${pageId}/feed`, { limit: 1, fields: 'id,message,created_time' }, derivedPageAccessToken)) : { ok: false, error: 'No derived Page access token available' };
  return json(res, 200, { ok: true, connected: true, deployed: 'meta-page-token-derive-v12', pageId, me, permissions, page, pageFeedRead, configuredPage, hasDerivedPageAccessToken: Boolean(configuredPage.ok && configuredPage.result && configuredPage.result.access_token), configuredFeedRead });
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    return json(res, 404, { ok: false, error: 'Unknown API route', path, deployed: 'meta-page-token-derive-v12' });
  } catch (error) {
