...
async function handleTikTokLoginDebug(req, res) {
  ...
  return json(res, 200, {
    ok: true,
    deployed: 'meta-platform-slot-hours-v40',
...
async function publishReadyPayload(body) {
  ...
  return { ok, deployed: 'meta-platform-slot-hours-v40', taskId: body.taskId || null, results, publishedAt: new Date().toISOString() };
}
...
async function handleReadyPublish(req, res) {
  ...
  try {
    ...
    const result = await publishReadyPayload(body);
    return json(res, result.ok ? 200 : 502, result);
  } catch (error) {
    return json(res, 400, { ok: false, deployed: 'meta-platform-slot-hours-v40', error: error.message || 'Publish failed' });
  }
}
...
async function handleAutoPublish(req, res) {
  ...
  return json(res, 200, { ok: true, deployed: 'meta-platform-slot-hours-v40', checked: all.length, due: due.length, processed, ranAt: now.toISOString() });
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    return json(res, 404, { ok: false, error: 'Unknown API route', path, deployed: 'meta-platform-slot-hours-v40' });
  } catch (error) {
