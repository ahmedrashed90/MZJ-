...
async function handleTikTokLoginDebug(req, res) {
  ...
  return json(res, 200, {
    ok: true,
    deployed: 'meta-publish-prep-search-structure-meta-v59',
...
async function publishReadyPayload(body, req = null) {
  ...
  return { ok, deployed: 'meta-publish-prep-search-structure-meta-v59', taskId: body.taskId || null, results, publishedAt: new Date().toISOString() };
}
...
async function handleReadyPublish(req, res) {
  ...
  try {
    ...
    return json(res, 400, { ok: false, deployed: 'meta-publish-prep-search-structure-meta-v59', error: error.message || 'Publish failed' });
  }
}
...
async function handleAutoPublish(req, res) {
  ...
  return json(res, 200, { ok: true, deployed: 'meta-publish-prep-search-structure-meta-v59', checked: all.length, due: due.length, processed, ranAt: now.toISOString() });
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    return json(res, 404, { ok: false, error: 'Unknown API route', path, deployed: 'meta-publish-prep-search-structure-meta-v59' });
  } catch (error) {
