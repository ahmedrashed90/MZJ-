...
async function handleTikTokLoginDebug(req, res) {
  ...
  return json(res, 200, {
    ok: true,
    deployed: 'meta-settings-lists-closed-v34',
...
async function handleReadyPublish(req, res) {
  ...
  return json(res, ok ? 200 : 502, { ok, deployed: 'meta-settings-lists-closed-v34', taskId: body.taskId || null, results, publishedAt: new Date().toISOString() });
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    return json(res, 404, { ok: false, error: 'Unknown API route', path, deployed: 'meta-settings-lists-closed-v34' });
  } catch (error) {
