...
function appendGraphParam(body, key, value) {
...
export async function graphPost(path, params = {}, token) {
  ...
  Object.entries(params || {}).forEach(([key, value]) => appendGraphParam(body, key, value));
  if (token) {
...
export async function getConfiguredPage() {
  ...
  // META_PAGE_ACCESS_TOKEN may be a System User token. If Meta exposes a real Page access token,
  // prefer it for publishing. Otherwise fall back to the configured token for diagnostics only.
  return { ...page, access_token: page.access_token || pageToken, _source: page.access_token ? 'page access token derived from configured token' : 'META_PAGE_ACCESS_TOKEN fallback' };
}
