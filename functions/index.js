...
exports.runAutoPublishNow = onRequest({
  ...
  try {
    const result = await runAutoPublishOnce();
    res.status(200).json({ ok: true, deployed: 'whatsapp-caption-icons-v69', ...result });
  } catch (error) {
    logger.error(error);
    res.status(500).json({ ok: false, deployed: 'whatsapp-caption-icons-v69', error: error.message || String(error) });
  }
});
