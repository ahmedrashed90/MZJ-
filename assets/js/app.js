...
function isCampaignContentWritingTask(task){
  ...
  const typeText = identityClean([
    task?.taskType, task?.contentType, task?.structureTaskLabel, task?.creative, task?.product, task?.sourceRequestStep, task?.campaignRequestBrief, task?.contentWriterBrief
  ].filter(Boolean).join(' '));
  const sectionRole = normalizeDepartmentRole(sectionText);
  const isContentSection = sectionRole === 'content' || sectionText.includes('محتوي') || sectionText.includes('كاتب') || sectionText.includes('content') || sectionText.includes('writer');
  const isCampaignWriting =
    typeText.includes('كتابه محتوي حمله') ||
    typeText.includes('كتابه محتوي') ||
    (typeText.includes('كتابه') && typeText.includes('محتوي')) ||
    typeText.includes('campaign request data') ||
    typeText.includes('campaign content') ||
    typeText.includes('content writing') ||
    task?.needsStructureUpload ||
    task?.structureDeadline ||
    task?.sourceRequestStep === 'campaign_request_data';
  return Boolean(isCampaignWriting && (isContentSection || sectionRole === 'other' || task?.needsStructureUpload || task?.structureDeadline || task?.sourceRequestStep === 'campaign_request_data'));
}
...
function renderStructureSection(task){
  ...
  const isStructureCarrierTask = isCampaignStructureTask(task) || contentWritingTask || task?.needsStructureUpload || task?.structureDeadline || task?.sourceRequestStep === 'campaign_request_data';
  ...
  const canUpload = !admin && (contentWritingTask || isStructureCarrierTask) && status !== 'approved' && status !== 'distributed';
...
function buildTaskDetailHtml(task){
  ...
  const quickStructureUpload = (!admin && isContentWriting && !contentHasStructure && contentStructure.status !== 'approved' && contentStructure.status !== 'distributed')
...
function isCampaignContentWritingPrepTask(task){
  ...
  return text.includes(identityClean('كتابة محتوى حملة')) || text.includes('campaigncontentwriting') || text.includes('contentwritingcampaign');
}
