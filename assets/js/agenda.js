(function(){
  ...
  const state={step:1,data:{month:'',name:'',start:'',end:''},days:[],created:false,creating:false,agendaId:'',campaignId:'',agendaCode:'',modalDay:null,activeTaskId:null,openTaskId:null,rawFolderResult:null,draftLoaded:false,draftLoading:false,draftSaving:false,draftSavedAt:''};
  ...
  function buildAgendaPublishSchedule(){
    ...
    state.days.forEach(day=>(day.tasks||[]).forEach((t,taskIndex)=>{
      ...
      rows.push({
        ...
        creativeCode:t.code,
        source:'agenda',
        contentReceiptDate:t.contentReceiptDate||day.date,
        executionReceiptDate:t.executionReceiptDate||day.date,
        platforms,
  ...
  function validationErrors(){
    ...
    state.days.forEach(day=>(day.tasks||[]).forEach(t=>{
      ...
      if(!t.contentReceiptDate)errors.push(`${label}: اختر تاريخ الاستلام من يوزرات المحتوى.`);
      if(!t.executionReceiptDate)errors.push(`${label}: اختر تاريخ الاستلام من يوزرات التنفيذيين.`);
  ...
  const AGENDA_DRAFTS_COLLECTION=window.MZJ_AGENDA_DRAFTS_COLLECTION||'marketing_agenda_drafts';
  const AGENDA_DRAFT_LOCAL_PREFIX='mzj_agenda_draft_v38_';
  function agendaDraftOwnerId(){const c=ctx(),u=c.currentUser||{};try{const auth=window.mainAuth?.currentUser||window.auth?.currentUser||{};return String(auth.uid||auth.email||u.uid||u.id||u.email||u.name||'default-user').trim()}catch(_){return String(u.uid||u.id||u.email||u.name||'default-user').trim()}}
  function agendaDraftOwnerKey(){return agendaDraftOwnerId().toLowerCase().replace(/[^a-z0-9@._-]+/g,'-').replace(/^-+|-+$/g,'').slice(0,150)||'default-user'}
  function agendaDraftLocalKey(){return AGENDA_DRAFT_LOCAL_PREFIX+agendaDraftOwnerKey()}
  function agendaCleanDraft(value){try{return typeof cleanFirestoreValue==='function'?cleanFirestoreValue(value):JSON.parse(JSON.stringify(value))}catch(_){return value}}
  function syncAgendaVisibleFields(){const month=document.getElementById('agMonth'),name=document.getElementById('agName'),start=document.getElementById('agStart'),end=document.getElementById('agEnd');if(month)state.data.month=month.value;if(name)state.data.name=name.value.trim();if(start)state.data.start=start.value;if(end)state.data.end=end.value}
  function agendaDraftButton(){return `<button class="agenda-btn light" id="agSaveDraft" ${state.draftSaving?'disabled':''}>${state.draftSaving?'جاري حفظ المسودة...':'💾 حفظ مسودة'}</button>${state.draftSavedAt?`<small class="agenda-draft-saved">آخر حفظ: ${esc(new Date(state.draftSavedAt).toLocaleString('ar-SA'))}</small>`:''}`}
  async function saveAgendaDraft(){
    if(state.draftSaving)return;syncAgendaVisibleFields();state.draftSaving=true;render();
    try{const c=ctx();const stamp=new Date().toISOString();const payload=agendaCleanDraft({ownerId:agendaDraftOwnerId(),ownerKey:agendaDraftOwnerKey(),step:state.step,data:state.data,days:state.days,agendaId:state.agendaId||'',campaignId:state.campaignId||'',agendaCode:state.agendaCode||'',rawFolderResult:state.rawFolderResult||null,status:'draft_builder',savedAt:stamp,updatedAt:stamp});if(c.db)await c.db.collection(AGENDA_DRAFTS_COLLECTION).doc(agendaDraftOwnerKey()).set(payload);try{localStorage.setItem(agendaDraftLocalKey(),JSON.stringify(payload))}catch(_){}state.draftSavedAt=stamp;try{if(typeof showToast==='function')showToast('تم حفظ مسودة الأجندة ويمكنك الرجوع لاستكمالها في أي وقت.');else alert('تم حفظ مسودة الأجندة.')}catch(_){}}
    catch(e){console.error('Agenda draft save failed',e);alert('تعذر حفظ مسودة الأجندة: '+(e.message||e))}finally{state.draftSaving=false;render()}
  }
  function applyAgendaDraft(data){if(!data||typeof data!=='object')return false;state.step=Math.min(3,Math.max(1,Number(data.step)||1));state.data={...state.data,...(data.data||{})};state.days=Array.isArray(data.days)?data.days:[];state.agendaId=String(data.agendaId||'');state.campaignId=String(data.campaignId||'');state.agendaCode=String(data.agendaCode||'');state.rawFolderResult=data.rawFolderResult||null;state.created=false;state.creating=false;state.draftSavedAt=String(data.savedAt||data.updatedAt||'');return true}
  async function loadAgendaDraft(){if(state.draftLoaded||state.draftLoading)return;state.draftLoading=true;try{const c=ctx();let data=null;if(c.db){const snap=await c.db.collection(AGENDA_DRAFTS_COLLECTION).doc(agendaDraftOwnerKey()).get();if(snap?.exists)data=snap.data()||{}}if(!data){try{const raw=localStorage.getItem(agendaDraftLocalKey());if(raw)data=JSON.parse(raw)}catch(_){}}if(applyAgendaDraft(data)){try{if(typeof showToast==='function')showToast('تم استرجاع مسودة الأجندة المحفوظة.')}catch(_){}}}catch(e){console.warn('Agenda draft load skipped',e)}finally{state.draftLoaded=true;state.draftLoading=false;render()}}
  async function deleteAgendaDraft(){try{const c=ctx();if(c.db)await c.db.collection(AGENDA_DRAFTS_COLLECTION).doc(agendaDraftOwnerKey()).delete()}catch(e){console.warn('Agenda draft delete skipped',e)}try{localStorage.removeItem(agendaDraftLocalKey())}catch(_){}state.draftSavedAt=''}
  ...
  function taskCard(t){
    ...
    const modalDay=state.days.find(d=>d.date===state.modalDay);
    const contentReceiptDate=t.contentReceiptDate||modalDay?.date||'';
    const executionReceiptDate=t.executionReceiptDate||modalDay?.date||'';
    ...
    return `<article class="agenda-task-card ${state.activeTaskId===t.id?'active-task':''} ${open?'is-open':'is-collapsed'} ${touched?'has-selection':'no-selection'}" data-task="${t.id}">
      ...
      <div class="agenda-task-body" ${open?'':'hidden'}>
        <div class="agenda-task-deadlines">
          <label class="agenda-deadline-field"><span>تاريخ الاستلام من يوزرات المحتوى</span><input class="agenda-input" type="date" data-agenda-receipt-date="content" data-task-id="${t.id}" value="${esc(contentReceiptDate)}"></label>
          <label class="agenda-deadline-field"><span>تاريخ الاستلام من يوزرات التنفيذيين</span><input class="agenda-input" type="date" data-agenda-receipt-date="execution" data-task-id="${t.id}" value="${esc(executionReceiptDate)}"></label>
        </div>
  ...
  function bindModal(){
    ...
    document.querySelectorAll('[data-agenda-receipt-date]').forEach(input=>input.onchange=()=>{const t=getTask(day,input.dataset.taskId);if(!t)return;if(input.dataset.agendaReceiptDate==='content')t.contentReceiptDate=input.value;else t.executionReceiptDate=input.value});
  ...
  function buildDashboardTasks(campaignId,campaignCode){
    ...
    pairRows().forEach(pair=>{
      ...
      const contentReceiptDate=t.contentReceiptDate||day.date;
      const executionReceiptDate=t.executionReceiptDate||day.date;
  ...
  async function createAgenda(){
    ...
    try{
      if(c.db){
        ...
        const batch=c.db.batch();batch.set(agendaRef,agendaPayload);batch.set(campaignRef,campaignPayload);await batch.commit();await deleteAgendaDraft();
      }else{state.agendaId='LOCAL-'+Date.now();state.campaignId='agenda_'+state.agendaId;state.agendaCode=`AGENDA-${state.data.month.replace('-','')}-${String(Date.now()).slice(-6)}`;await deleteAgendaDraft()}
  ...
  async function taskWorkbook(pair){
    ...
    setStyledValue(ws,7,'موعد تسليم التاسك',t.contentReceiptDate||day.date);
  ...
  function renderAgendaWithDraft(){render();if(!state.draftLoaded&&!state.draftLoading)setTimeout(loadAgendaDraft,30)}
  window.MZJAgendaRender=renderAgendaWithDraft;
  window.addEventListener('hashchange',()=>{if(location.hash==='#create-agenda')renderAgendaWithDraft()});
})();
